...
void cdcacm_init(void)
{
	...
	USB_GPCS = 3; // Force USB device mode.
}
...
{
        volatile uint32_t *unique_id_p = (volatile uint32_t *)0x400fe1e0;
	uint32_t unique_id = (unique_id_p[0] & 0x7fffffff) +
			(unique_id_p[1] & 0x7fffffff);
        int i;
