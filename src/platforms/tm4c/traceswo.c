...
void traceswo_init(void)
{
	...
	//gpio_unlock_commit(SWO_PORT, GPIO7);

	gpio_mode_setup(SWO_PORT, GPIO_MODE_INPUT, GPIO_PUPD_NONE, SWO_PIN);
	//gpio_mode_setup(SWO_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, GPIO7);
	gpio_set_af(SWO_PORT, 1, SWO_PIN); /* U2RX */
	//gpio_set_af(SWO_PORT, 1, GPIO7); /* U2TX */

	uart_disable(TRACEUART);

	/* Setup UART parameters. */
	uart_clock_from_sysclk(TRACEUART);
	//UART_CTL(TRACEUART) |= UART_CTL_HSE;
	//uart_set_baudrate(TRACEUART, 800000 >> 1);
...
void trace_buf_push(void)
{
	...
	if (usbd_ep_write_packet(usbdev, 0x85, &buf_rx[buf_rx_out], len) == len) {
		buf_rx_out += len;
